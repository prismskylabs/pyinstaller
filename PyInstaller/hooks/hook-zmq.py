...
def hook(mod):
    ...
    for libname in ('libzmq', 'libsodium', 'zmq', 'zmq.libzmq'):
        print "Directory: {}".format(zmq_directory)
        for f in glob.glob(os.path.join(zmq_directory, libname + '*.{pyd,so,dll,dylib}*')):
            print "File: {}".format(f)
